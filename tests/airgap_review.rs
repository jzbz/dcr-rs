...
mod fp_compat {
    ...
    /// 7-element package generated by the original Pulse interop format.
    ...
    /// The same package with an appended fingerprint [1,2,3,4].
    ...
    fn legacy_seven_element_decodes_with_none() {
        let req = decode_sign_request(&hex::decode(LEGACY7).unwrap()).expect("legacy must decode");
    ...
    fn eight_element_decodes_with_fingerprint() {
        let req = decode_sign_request(&hex::decode(WITHFP).unwrap()).expect("v1+fp must decode");
    ...
    fn encode_roundtrips_both_ways() {
        let mut req = decode_sign_request(&hex::decode(WITHFP).unwrap()).unwrap();
        let bytes = encode_sign_request(&req).unwrap();
        assert_eq!(
            decode_sign_request(&bytes).unwrap().account_fp,
            Some([1, 2, 3, 4])
        );
        // Dropping the fingerprint reproduces the legacy 7-element bytes
        // exactly — old decoders never see the new field.
        req.account_fp = None;
        let bytes = encode_sign_request(&req).unwrap();
        assert_eq!(hex::encode(&bytes), LEGACY7);
...
fn validate_refuses_duplicate_inputs_and_oversized_packages() {
    ...
    // The same outpoint listed twice inflates the apparent input total (the
    // fee would display as 110_000 in, 90_000 out) — must refuse.
    let mut req = basic_request(foreign_script(0x11), outputs.clone());
