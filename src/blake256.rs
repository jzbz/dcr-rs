...
//! primitive in this crate, so it is vendored here and pinned by known-answer
//! vectors generated from dcrd's `crypto/blake256` Go package for every
//! padding path (see the test module), plus the transitive dcrd address/HD
//! reference vectors in `tests/vectors.rs`.
//!
...
///
/// ```
/// let mut h = dcr_rs::blake256::Blake256::new();
/// h.update(b"hello ");
/// h.update(b"world");
/// assert_eq!(h.finalize(), dcr_rs::blake256::sum256(b"hello world"));
/// ```
#[derive(Clone)]
pub struct Blake256 {
    h: [u32; 8],
